...
export default function Sidebar1(props) {
  ...
  return (
    <aside
      className={`${
        sidebarx
          ? "bg-azul-900 z-10 md:w-64 fixed top-0 left-0 w-full h-screen transform translate-x-0 transition  duration-300 ease-in-out"
          : "bg-azul-900 z-10 md:w-64 fixed top-0 left-0 w-full h-screen transform -translate-x-full transition duration-300 ease-in-out"
      }`}
    >
        ...
        <div className="w-full mt-2 ">
          {links.map((ev) => (
            <Link key={ev.id} href={ev.url}>
              <a className="nav-link text-indigo-300 hover:bg-blue-700 hover:text-white px-6 ">
